import {lateBot, morningBot} from "./../../netlify/services/discord/bot/schedule";
import {onboardingMessage} from "./../../netlify/services/discord/bot/utils";
import {Person, User} from "./../../netlify/services/types";
    ...
    .onCreate(async (snapshot, context) => {
      ...
      if (vote) {
        ...
        const snap = await getFirestore()
            .collection(`/people/${personId}/votes`).get();
...
export const onCreateUser = firestore
    .document("/users/{uid}").onCreate(async (snapshot) => {
...
export const onCreateDiscord = firestore
    .document("/discord/{uid}").onCreate(async (snapshot, context) => {
...
export const onUpdateDiscord = firestore
    .document("/discord/{uid}").onUpdate(async (snapshot, context) => {
      const {uid} = context.params;
...
export const onUpdatePeople = firestore
    .document("/people/{personId}").onUpdate(async (snapshot, context) => {
